...
public class AiAgentConfig {
    ...
    public static final String JOB_STANDARD_VECTOR_TABLE = "job_standard_vector_store";
    public static final String DOCUMENT_VECTOR_TABLE = "document_vector_store";
    ...
    public PgVectorStore pgVectorStore(@Value("${spring.ai.openai.base-url}") String baseUrl,
        ...
        OpenAiEmbeddingModel embeddingModel = createEmbeddingModel(
                baseUrl, apiKey, embeddingModelId, embeddingModelName, completionsPath, embeddingsPath);
    ...
    @Bean("jobStandardVectorStore")
    public PgVectorStore jobStandardVectorStore(@Value("${spring.ai.openai.base-url}") String baseUrl,
                                                @Value("${spring.ai.openai.api-key}") String apiKey,
                                                @Value("${spring.ai.openai.embedding-model-id:2005}") String embeddingModelId,
                                                @Value("${spring.ai.openai.embedding-model:text-embedding-3-small}") String embeddingModelName,
                                                @Value("${spring.ai.openai.completions-path:v1/chat/completions}") String completionsPath,
        ...
        return PgVectorStore.builder(jdbcTemplate, embeddingModel)
                .vectorTableName(JOB_STANDARD_VECTOR_TABLE)
                .dimensions(RESUME_EMBEDDING_DIMENSIONS)
                .initializeSchema(true)
                .build();
    }
    ...
    public PgVectorStore documentVectorStore(@Value("${spring.ai.openai.base-url}") String baseUrl,
                                             @Value("${spring.ai.openai.api-key}") String apiKey,
                                             @Value("${spring.ai.openai.embedding-model-id:2005}") String embeddingModelId,
                                             @Value("${spring.ai.openai.embedding-model:text-embedding-3-small}") String embeddingModelName,
                                             @Value("${spring.ai.openai.completions-path:v1/chat/completions}") String completionsPath,
        ...
        return PgVectorStore.builder(jdbcTemplate, embeddingModel)
                .vectorTableName(DOCUMENT_VECTOR_TABLE)
                .dimensions(RESUME_EMBEDDING_DIMENSIONS)
                .initializeSchema(true)
                .build();
    }
    ...
    private OpenAiEmbeddingModel createEmbeddingModel(String baseUrl,
                                                      String apiKey,
